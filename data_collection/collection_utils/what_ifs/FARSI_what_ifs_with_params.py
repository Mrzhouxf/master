...
def run(check_points_start, check_points_top_folder, previous_results):
    # 这段Python代码定义了一个函数 run，该函数的主要功能是执行一个模拟硬件系统性能和功耗的流程。
    # 这个流程涉及多个参数，包括硬件的工作负载、模拟深度、频率范围、预算缩放比例等。
    # 代码的主要部分是循环结构，它遍历不同的参数组合并调用一个名为 run_with_params 的函数来执行模拟。
    ...
    if check_points_start:
        ...
        all_dirs = [x[0] for x in os.walk(check_points_top_folder)]#当前文件夹下的所有文件夹路径，包括当前文件夹
...
def create_final_folder(run_folder):
    # 当你调用 create_final_folder 函数并提供一个文件夹路径时，它会将该文件夹重命名为一个新的文件夹，
    # 新文件夹的名称是在原文件夹名前加上 “final_” 前缀。
    # 例如，如果你提供的是 /home/user/my_folder，那么新文件夹的路径将是 /home/user/final_my_folder。
    source = run_folder
...
def run_batch(check_points_start, check_points_top_folder):
    # 它的目的是执行一个批处理优化过程。这个过程包括从检查点开始（如果设置了check_points_start），执行优化，处理内存不足的情况，并最终汇总结果。
...
# 它的目的是找到所有包含"final"的文件夹，这些文件夹位于配置中指定的home_dir下的"data_collection/data/simple_run"目录中。这些文件夹通常包含优化过程的最终结果。
